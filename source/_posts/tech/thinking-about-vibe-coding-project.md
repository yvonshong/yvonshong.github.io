...
项目的主要工作不是系统架构，而是调整缩放、拖动、点击和移动等操作，让手势反馈更加自然。
...
这类工作虽然不是从零创造一个新产品，却降低了重新理解旧项目的成本，也让过去的积累重新获得了展示和使用价值。
...
Agent 可以帮助我理解 Obsidian 的插件体系和 API，完成文档读取、语义分类和链接生成。

但真正的问题并不是“能不能生成双向链接”，而是“什么样的链接才有意义”。

如果只是机械地批量生成链接，而没有 LLM 对内容语义的理解，也没有人对知识结构进行约束，那么自动化反而可能制造出一个更加混乱的知识网络。

因此，人需要定义更高层的结构，包括分类标准、知识体系的组织方式，以及在 LLM 语义推理、脚本化执行和人工控制之间怎样划分边界。
...
后续可能还需要在数据库或 Vercel 中补充更多业务数据，但目前还没有完全确定具体方式。这些内容更适合根据实际用户和业务量，在后期逐步加入，而不是一开始就建立过于复杂的数据系统。
...
很多工具并不是一开始就计划开发的，而是在业务推进过程中才发现必须存在。

为了导入交通数据，需要批量处理脚本；为了生产套票页面，需要 Admin 内容工具；为了做多语言营销，需要翻译和内容转换能力；为了衡量效果，需要埋点和数据分析；为了管理分佣，需要 Affiliate 管理工具。

这些工具并不是为了让技术架构显得更复杂，而是因为业务流程中真实存在摩擦。

判断一个工具是否有价值，应该看它是否解决了当前业务中的真实问题，而不是它是否足够先进。
...
真正重要的不是“设计得越多越好”，而是判断哪些决策一旦错误，会产生最大的返工成本。
...
换句话说，软件工程的关键并不是消除变化，而是管理变化；不是消除复杂度，而是在不可避免的复杂度中建立结构化的秩序。
